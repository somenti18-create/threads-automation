...
「先週〇〇さんの〇〇をやった」「今日〇〇をやってみた」から始める。
...
「〇〇をやってみて気づいたこと。」「今日面白いことがあった。」から始めてもOK。
→ 状況・実験の説明 → 結果（数字あれば最高） → そこから得た気づき
結論を押しつけない。「〜なんですよね」「〜だったりする」で終わる。150〜400文字"""
    },
    {   # JST 10:00 繋がり
...
→ 相談内容（数字・状況） → 何を変えたか（詳細は言わない） → 変化後の事実（数字）
「新しい客を増やしたんじゃなくて〜」のような気づきで締める。150〜400文字"""
    },
    {   # JST 13:00 DM誘導
        "type": "dm", "label": "DM誘導型",
...
自分のアカウントやクライアント運用での実験結果を語る。
→ 仮説 → やったこと → 結果（数字） → 解釈
「これが全てじゃないけど〜」のような余白を残して終わる。150〜400文字"""
    },
    {   # JST 17:00 本音・失敗
        "type": "viral", "label": "本音型",
        "description": """正直な本音・失敗・葛藤を語る。「正直〜」「ぶっちゃけ〜」から始めてもOK。
自動化・SNS運用・起業に関するリアルな話。うまくいかなかった話もOK。
→ 状況 → 正直な感情 → でも気づいたこと・続けてる理由
...
毎回違うエピソードを選ぶ：公務員時代の話・転売の失敗・動画編集の経験・起業を決めた瞬間。
