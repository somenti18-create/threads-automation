...
# 時間帯固定の投稿タイプ（JST順）
# UTC: ["22:00","23:30","01:00","02:30","04:00","06:00","08:00","10:00","12:00","13:30"]
# JST: [ 07:00,  08:30,  10:00,  11:30,  13:00,  15:00,  17:00,  19:00,  21:00,  22:30]
POST_TYPES_BY_SLOT = [
    {   # JST 07:00 仕事の実体験
...
押しつけNG。自分の体験を淡々と共有するスタンス。150〜400文字"""
    },
    {   # JST 08:30 気づき・発見
        "type": "viral", "label": "気づき型",
        "topic_hint": "未来を想像させる / 情報より感情が動かす / 価格より価値を伝える からランダムに選ぶ",
        "hook_hint": "逆張り型（「SNSを頑張るほど売上が下がることがある」）か問いかけ型（断言）",
        "description": """仕事や運用の中で得た気づき・発見を語る。
冒頭2行：逆張り型（「SNSを頑張るほど〜」「発信量を増やして売上が下がった。」）か問いかけ型断言（「なぜ技術があるのに予約が入らないのか。」）で始める。
テーマ例：「機能じゃなく未来を見せる」「有益な情報より感情が売上を動かす」「価値が伝わらないから高いと言われる」
→ 状況・実験 → 結果（数字あれば最高） → 気づき
結論を押しつけない。「〜なんですよね」で終わる。150〜400文字"""
    },
    {   # JST 10:00 繋がり
        "type": "connect", "label": "繋がり投稿（問いかけ型）",
        "hook_hint": "ターゲット直撃型（「整体師さんへ。」）で始める",
        "description": """整体・サロン・パーソナルトレーナーでSNSや集客に悩んでる方に向けた繋がり投稿。
冒頭2行：「整体師さんに聞いてほしい話。」「サロンオーナーさんへ。」など職種を直撃して始める。
冒頭で自分の仕事内容や支援実績にさらっと触れてから繋がりを誘う。
締めは「気軽にお話しましょう！」か「困ってる人も繋がりましょう！お話聞くだけでも全然大丈夫です！」150〜300文字"""
    },
    {   # JST 11:30 クライアント事例
...
「〜したんじゃなくて〜しただけ」のような気づきで締める。150〜400文字"""
    },
    {   # JST 13:00 DM誘導
        "type": "dm", "label": "DM誘導型",
        "hook_hint": "ターゲット直撃型か逆張り型で始める",
        "description": """自分の仕事内容に触れながら「話を聞きたい」でDMに誘導する。
冒頭2行：「SNSを頑張ってるのに予約が入らない、その理由はたいてい同じです。」などターゲットの悩みを直撃して始める。
→ 自分が何をやってる人かさらっと説明
→ どんな人の悩みを解決してきたか（具体的に）
→ 締め：「気軽にご連絡ください！」（これ以外NG）。押し売りゼロ。150〜300文字"""
    },
    {   # JST 15:00 実験・検証
        "type": "viral", "label": "実験型",
        "topic_hint": "SNS数字との向き合い方 / 情報より感情が動かす / 自動化の話 からランダムに選ぶ",
        "hook_hint": "逆張り型か落差型で始める",
        "description": """「〇〇を試してみた」「〇〇と〇〇を比較してみた」形式の実験・検証の話。
冒頭2行：「"役立つ情報"より"失敗の話"の方がリーチ3.2倍だった。」など結果を先に出して引く。
テーマ例：「有益系投稿vs本音投稿を比較した」「投稿時間を変えたら〇〇が変わった」「LINE自動配信のタイミングを変えた実験」
→ 仮説 → やったこと → 結果（数字） → 解釈
「これが全てじゃないけど〜」のような余白を残して終わる。150〜400文字"""
    },
    {   # JST 17:00 本音・失敗
...
→ 状況 → 正直な感情 → でも気づいたこと
共感を押しつけない。「〜なんですよね」で自然に締める。150〜400文字"""
    },
    {   # JST 19:00 繋がり
        "type": "connect", "label": "繋がり投稿（宣言型）",
        "hook_hint": "数字で始める型か逆張り型",
        "description": """自分が何をやってる人かを宣言して繋がりを誘う。
冒頭2行：「月商70万が700万になった整体師さんを支援しています。」など実績から入るか、「フォロワー数を増やすより大事なことがある、と思っています。」など信念から入る。
整体・サロン・パーソナルトレーナーの支援実績に触れながら。
→ 自分のビジョン・やりたいこと → どんな人と繋がりたいか
締めは「困ってる人も繋がりましょう！お話聞くだけでも全然大丈夫です！」150〜300文字"""
    },
    {   # JST 21:00 ストーリー（キャリア）
        "type": "viral", "label": "ストーリー型",
        "hook_hint": "落差型か体験の入口型。公務員→副業転々→起業の流れ",
        "description": """元公務員→副業転々→起業という自分のキャリアストーリーを語る。
冒頭2行：「公務員を辞めた理由は、給料が安かったからじゃない。」「転売で月3万稼いだとき、会社員の30万より重かった。」など落差や本音から入る。
毎回違うエピソードを選ぶ：公務員時代の話・転売の失敗・動画編集の経験・起業を決めた瞬間・最初のクライアント獲得。
→ 当時の状況（一人称） → 感情・葛藤 → どうなったか
「結局〜だった。」など断定で終わる。説明しすぎない。150〜400文字"""
    },
    {   # JST 22:30 繋がり
        "type": "connect", "label": "繋がり投稿（共感型）",
        "hook_hint": "ターゲット直撃型か問いかけ型（断言）",
        "description": """整体・サロン・PT向けの悩みに共感しながら繋がりを誘う。
冒頭2行：「技術があるのに予約が入らない。それ、発信の問題じゃないかもしれません。」「値上げしたいけど怖い、という声をよく聞きます。」など悩みを直撃して始める。
→ よく聞く悩み（具体的） → 自分の見解（押しつけNG） → 話したい
締めは「気軽にお話しましょう！」か「困ってる人も繋がりましょう！お話聞くだけでも全然大丈夫です！」150〜300文字"""
    },
]
